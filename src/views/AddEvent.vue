...
<template>
  <Transition name="fade">
    ...
    >
      ...
      >
        <div
          class="px-6 py-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50"
        >
          <div>
            <h3 class="text-lg font-bold text-slate-800">Nouvelle tâche</h3>
            <p class="text-xs text-indigo-600 font-bold uppercase tracking-widest">
              {{ modalStore.selectedDay }}
            </p>
          </div>
          <button
        ...
        <div
          class="px-6 py-4 bg-slate-50/50 flex flex-col-reverse sm:flex-row sm:justify-end gap-3"
        >
          <button
            @click="modalStore.close()"
